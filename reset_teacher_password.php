...
if(!isset($_GET["code"])){
 ...
 $getEmailQuery  = mysqli_query($conn,  "SELECT email FROM reset_admin_password WHERE code='$code' ");//studentlogin table
...
if(isset($_POST["password"])) {
    ...
    $email = $row["email"];
    $query = mysqli_query($conn,"UPDATE users SET password = '$hash' WHERE username = '$email'");

    if($query){
    $query = mysqli_query($conn, "DELETE FROM reset_admin_password WHERE code='$code'");//another table
